               ...
               <div class="col-9">
                 ...
                 <nav aria-label="breadcrumb">
                   <ol class="breadcrumb">
                     ...
                     <li class="breadcrumb-item" aria-current="page">회의실</li>
                   </ol>
                 </nav>
               </div>
               <div class="col-3">
        ...
        <div class="card w-100 position-relative overflow-hidden">
			...
			<!-- 상태별 카운트 위젯 끝 -->
                ...
                <div class="row mb-4 align-items-center">
                    <div class="col-md-3">
                        <form id="filter-form" action="/management/reserve/meetingList" method="get">
               ...
               <div class="col-md-9 d-flex justify-content-end align-items-center" >
                   <form action="/management/reserve/meetingList" method="get" class="position-relative me-2" id="search-form">
                ...
                <!-- 셀렉트 및 검색창 끝 -->
         <form action="/management/reserve/excelMeetingDown" method="post" id="excelDownForm">
                ...
                <table class="table text-nowrap mb-0 align-middle" style="table-layout: fixed;">
                    ...
                    <thead class="text-dark fs-4">
                        <tr>
                            ...
                            <th><h6 class="fs-4 fw-semibold mb-0">회의실명</h6></th>
                            ...
                            <th><h6 class="fs-4 fw-semibold mb-0">예약시간</h6></th>
                    ...
                    <tbody>
                        <c:choose>
                            <c:when test="${empty pagingVO.dataList}">
                                <tr>
                                    <td colspan="7">회의실 예약 내역이 존재하지 않습니다.</td>
                                </tr>
                            </c:when>
                            <c:otherwise>
                                <c:forEach items="${pagingVO.dataList}" var="mr" varStatus="status">
                                    <c:if test="${mr.mrrStatus == selectedItem or empty selectedItem}">
                                        <tr>
                                            <td><p class="mb-0 fw-normal fs-4">${mr.mrrNo}</p></td>
                                            <td><p class="mb-0 fw-normal fs-4">${mr.mrName}</p></td>
											<td><p class="mb-0 fw-normal fs-4">${mr.mrrName}</p></td>
											<td><p class="mb-0 fw-normal fs-4">${mr.mrrTel}</p></td>
											<td><p class="mb-0 fw-normal fs-4">${fn:split(mr.mrrDate, " ")[0]}</p></td>
											<td><p class="mb-0 fw-normal fs-4">${mr.mrrTime}</p></td>
											<td><p class="mb-0 fw-normal fs-4"> <fmt:formatNumber value="${mr.mrrCost}" type="number" pattern="#,##0" /></p></td> 
                                            <td>
                                                <c:choose>
                                                    <c:when test="${mr.mrrStatus == 'N'}">
                                                        <span class="badge bg-danger-subtle text-danger" style="margin-left:10px">예약취소</span>
                                                    </c:when>
                                                    <c:when test="${mr.mrrStatus == 'Y'}">
                                                        <span class="badge bg-primary-subtle text-primary btn2" style="margin-left:10px" data-action="approve" data-mrrno="${mr.mrrNo }">예약완료</span>
                                                    </c:when>
                                                </c:choose>
                                            </td>
                                        </tr>
           ...
           <input type="submit" class="btn btn-primary" value="엑셀 다운로드" id="excelDownBtn" />
                ...
                <div class="text-end mb-3">
                    <c:if test="${not empty searchWord}">
                        <a href="/management/reserve/meetingList" class="btn btn-outline-primary me-3">목록</a>
                    </c:if>
                </div>
				...
				<!-- Pagination 끝 -->
				<form method="post" action="/management/reserve/meetingStatus" id="meetingStatus">
            </form>
            </div>
        </div>
    </div>
    ...
    $('.btn2').on('click', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
            	var mrrHtml = "<input type='text' name='mrrNo' value='" + mrrN + "' />";
            	$('#meetingStatus').append(mrrHtml);
            	$('#meetingStatus').submit();
